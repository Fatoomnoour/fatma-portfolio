...
export default function StatsStrip() {
  return (
    <section id="highlights" aria-labelledby="highlights-title" className="relative border-y theme-border section-bg-alt">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        <h2 id="highlights-title" className="sr-only">Portfolio highlights</h2>
        <div className="grid md:grid-cols-3 divide-y md:divide-y-0 md:divide-x theme-divider">
          {stats.map((stat, index) => {
            const Icon = stat.icon;
            return (
              <motion.div key={stat.label} initial={{ opacity: 0, y: 12 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true, amount: 0.4 }} transition={{ duration: 0.35, delay: index * 0.08 }} className="flex items-center gap-4 px-4 py-4 md:py-2 md:justify-center">
