...
export default function Hero() {
  return (
    <section id="home" className="relative flex flex-col items-stretch pt-20 sm:pt-24 pb-16 sm:pb-20 overflow-hidden hero-grid" aria-labelledby="hero-title">
      ...
      <div className="relative z-10 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 w-full">
        <div className="grid lg:grid-cols-[minmax(0,1fr)_minmax(0,1fr)] gap-10 xl:gap-16 items-center">
          ...
          <motion.div initial={{ opacity: 0, x: 24 }} animate={{ opacity: 1, x: 0 }} transition={{ duration: 0.6, delay: 0.12 }} className="relative">
            <div className="technical-panel p-4 sm:p-6">
              <div className="flex items-center justify-between pb-4 border-b theme-divider gap-4">
                <div>
              ...
              <div className="data-flow" aria-label="Example data engineering flow">
                {flow.map((stage, index) => {
                  const Icon = stage.icon;
                  return (
                    <div key={stage.label} className="data-flow-stage hero-flow-row">
                      <div className="data-flow-card">
                        <span className="data-flow-number">0{index + 1}</span>
                        <span className="pipeline-icon data-flow-icon"><Icon className="w-4 h-4 text-primary-700 dark:text-primary-300" aria-hidden="true" /></span>
                        <span className="data-flow-copy">
                          <span className="block text-[10px] uppercase tracking-[0.14em] theme-text-muted">{stage.label}</span>
              ...
              <div className="grid grid-cols-3 gap-3 pt-4 mt-4 border-t theme-divider">
