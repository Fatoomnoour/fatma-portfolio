...
export default function PipelineConsole() {
  ...
  return (
    <section id="pipeline" className="relative py-24 section-bg-alt overflow-hidden">
      ...
      <div className="absolute right-0 top-1/4 w-96 h-96 bg-primary-500/5 rounded-full blur-[140px]" />

      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
        <div className="flex flex-col lg:flex-row lg:items-end lg:justify-between gap-8 mb-12">
          <div>
            <span className="eyebrow">Interactive Data Flow</span>
            <h2 className="section-title mt-3">
              See a pipeline <span className="text-gradient">think</span>.
            </h2>
        ...
        <div className="grid lg:grid-cols-[1.1fr_0.9fr] gap-8 items-stretch">
                  ...
                  return (
                    <button
                      ...
                      className={`relative w-full flex items-center gap-4 text-left p-4 rounded-xl border transition-all duration-200 active:scale-[0.99] ${
                        active ? 'border-primary-500/30 bg-primary-500/5 shadow-sm' : 'border-transparent hover:border-slate-300 hover:bg-slate-50 dark:hover:border-slate-700 dark:hover:bg-slate-800/50'
                      }`}
                    >
                      <span className={`relative z-10 w-10 h-10 rounded-xl flex items-center justify-center border ${active ? 'bg-white dark:bg-slate-800 border-primary-400' : 'bg-slate-50 dark:bg-slate-900 border-slate-200 dark:border-slate-700'}`}>
          ...
          >
            ...
            <div className="mt-10 border-t theme-divider pt-5">
              <p className="text-xs uppercase tracking-widest text-slate-500 mb-3">terminal output</p>
              <code className="block text-sm text-accent-600 dark:text-accent-400 font-mono leading-relaxed break-words">{stage.command}</code>
